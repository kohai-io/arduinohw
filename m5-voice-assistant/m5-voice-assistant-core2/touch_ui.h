...
void initTouchButtons() {
  // Main action buttons - extend to bottom of screen for easier tapping
  // Using darker colors for better contrast with white text
  touchButtons[BTN_VOICE] = {5, 155, 150, 80, "Voice", 0x0320, true};      // Dark green
  
  // Camera button only visible if camera is enabled
  #if ENABLE_CAMERA
  touchButtons[BTN_CAMERA] = {165, 155, 150, 80, "Camera", 0x0014, true};   // Dark blue
  #else
  touchButtons[BTN_CAMERA] = {165, 155, 150, 80, "Camera", 0x0014, false};  // Hidden when no camera
  #endif
...
void drawTouchButton(const TouchButton &btn) {
  ...
  // Draw button label with larger, bold text
  M5.Display.setTextColor(TFT_WHITE, btn.color);
  M5.Display.setTextDatum(MC_DATUM);
  M5.Display.setTextFont(4);  // Larger font for better readability
...
void drawScreenWithButtons(const String &text) {
  ...
  int textAreaHeight = HEIGHT - 85;  // Reserve 85px for buttons
